...
$plugin_disable = (zp_has_filter('admin_head') && extensionEnabled('admin-branding')) ? gettext_pl('Only one Zenphoto backend customization plugin may be enabled. Please disable the <code>admin-branding</code> plugin to use this one.','zp-branding') : '';
...
class zpBrandingOptions {
	...
	static function getOptionsSupported() {
		...
		if ( $zp_branding_logo ) {
			...
			$options = array( 
					gettext_pl('Logo for backend', 'zp-branding') => array('key' => 'zpbranding_logo-image', 'type' => OPTION_TYPE_RADIO,
						'order' => 1,
						'buttons' => array(
						...
						'desc' => gettext_pl('Choose what you want to use as logo or no logo at all.', 'zp-branding')),   
					gettext_pl('Select admin logo image', 'zp-branding') => array('key' => 'zpbranding_logo-custom', 'type' => OPTION_TYPE_CUSTOM, 
						'order' => 4, 
						'desc' => sprintf(gettext_pl('When <em>Custom logo</em> is selected for <em>Logo for backend</em> you may select a logo from files in the <em>%s</em> folder.<br>If you use the <em>elFinder</em> plugin for Uploads, you can upload files to this folder. Alternatively, you can use FTP to upload your image to the <em>adminlogo</em> folder.', 'zp-branding'),(UPLOAD_FOLDER.'/adminlogo/'))),
					gettext_pl('Width', 'zp-branding') => array('key' => 'zpbranding-width', 'type' => OPTION_TYPE_TEXTBOX,
						'order'=> 1,
						'desc' => gettext_pl('The width of the logo (px). Default is 200px.<br>The height is proportional.', 'zp-branding')),
					gettext_pl('Custom CSS', 'zp-branding') => array('key' => 'zpbranding_custom_css', 'type' => OPTION_TYPE_TEXTAREA, 
						'order' => 3,
						'multilingual' => 0,
